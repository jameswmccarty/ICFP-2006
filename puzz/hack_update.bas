...
DV       j = XLVIII
DX       k = XLVIII
DXV      IF i > pwdcount-I THEN GOTO DLXX
DXX      IF k > LVII THEN i = i + I
DXXV     IF k > LVII THEN k = XLVIII
DXXX     IF j > LVII THEN k = k + I
DXXXV    IF j > LVII THEN j = XLVIII
DXL      IF CHECKPASS(username, words(i)+CHR(j)+CHR(k)) THEN GOTO DLV
DXLV     j = j + I
DL       GOTO DXV
DLV      PRINT "found match!! for user " + username + CHR(X)
DLX      PRINT "password: " + words(i) + CHR(j) + CHR(k) + CHR(X)
DLXV     END
DLXX     PRINT "no complex matches for user " + username + CHR(X)
DLXXV    END
